...
function refreshMap() { setTimeout(()=> { try{ map.invalidateSize(); } catch(e){} }, 250); }
...
async function searchAddress() {
    ...
    if(address===""){ showToast("Ingresa una dirección","error"); return; }
    const btn=document.getElementById("btnSearch"); btn.disabled=true; btn.innerText="Buscando...";
    try{
        const formData=new FormData(); formData.append("address",address);
        ...
        if(!data.success){ 
            ...
            btn.disabled=false; btn.innerText="Buscar"; return;
        }
    ...
    } catch(e){ console.warn(e); showToast("Error al geocodificar","error"); }
...
function openEditModal(data){
    ...
    ["id","name","address","phone","company","latitude","longitude"].forEach(f=>document.getElementById(f).value=data[f==='id'?'ID_Point':f]||"");
    initMap();
    const lat=parseFloat(data.latitude)||32.5149, lon=parseFloat(data.longitude)||-117.0382;
    if(marker) marker.remove();
    marker=L.marker([lat,lon]).addTo(map);
...
function closeModal(event){ 
    if(event===true || (event.target && event.target.id==="modal")) document.getElementById("modal").style.display="none"; 
}
...
document.getElementById("branchForm").addEventListener("submit", async function(e){
    ...
    if(!lat || !lon){ showToast("Coordenadas obligatorias","error"); return; }
    ...
    } catch(e){ console.error(e); showToast("Error de red","error"); }
});

async function deleteBranch(id){
    if(!confirm("¿Eliminar esta sucursal?")) return;
    try{
        const res=await fetch("Branches_Delete.php",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:"id="+encodeURIComponent(id)});
        const data=await res.json();
        if(data.success){ showToast("Sucursal eliminada","success"); setTimeout(()=>location.reload(),700); }
...
function nextPage(companyId){ currentPage[companyId]=(currentPage[companyId]||1)+1; renderCompanyPage(companyId); }
function prevPage(companyId){ currentPage[companyId]=(currentPage[companyId]||1)-1; renderCompanyPage(companyId); }
...
async function autocompleteAddress(){
    ...
    if(!query || query.length<3){ box.style.display="none"; box.innerHTML=""; return; }
...
document.addEventListener("click", function(event){
    const box=document.getElementById("autocomplete-list");
    if(!event.target.closest("#autocomplete-list")&&!event.target.closest("#address")) box.style.display="none";
});
